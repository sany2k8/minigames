...
function Fib2048Solo({ seed, isBot, difficulty, paused, onScore, onDone }: SoloGameProps) {
            ...
            <div key={i} className="m2048-cell">
              {v > 0 && (
                <span
                  ...
                  style={{ background: bg, color: fg, fontSize: v >= 100 ? '0.8em' : '1em' }}
                >
                  {v}
                </span>
              )}
      ...
      {!isBot && (
        <>
          <div className="board-actions">
            <button className="btn btn-ghost" onClick={() => finish(score)}>End round</button>
          </div>
          <div className="hint">Merge consecutive Fibonacci tiles — 1+1=2, 2+3=5, 5+8=13…</div>
        </>
      )}
      {isBot && <div className="hint">🤖 chaining the sequence…</div>}
    </div>
  );
}

const mod: GameModule = { contest: 'score', Solo: Fib2048Solo };
export default mod;
